...
import { Eye, Clock, ArrowLeft, Sparkles, Award, Bookmark, Share2, Heart } from 'lucide-react';
...
export default function ArticleContent({ serverArticle, slug }) {
  ...
  useEffect(() => {
    // If the server didn't find it, try localStorage (articles published by demo users)
    if (!serverArticle) {
      try {
    ...
    try {
      ...
      if (bms.some(b => b.slug === slug)) setBookmarked(true);
  ...
  const handleToggleBookmark = () => {
    try {
      const bms = JSON.parse(localStorage.getItem('apex_bookmarks_v1') || '[]');
      const exists = bms.some(b => b.slug === slug);
      if (exists) {
        const filtered = bms.filter(b => b.slug !== slug);
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-6 border-y border-slate-100 dark:border-slate-800 mb-12 bg-slate-50/80 dark:bg-slate-900/60 px-6 sm:px-8 rounded-3xl shadow-sm">
          <div className="flex items-center gap-4 min-w-0">
            <img src={article.profiles?.avatar_url || 'https://api.dicebear.com/7.x/bottts/svg?seed=' + article.id} alt="Avatar" className="w-12 sm:w-14 h-12 sm:h-14 rounded-full object-cover shadow-md border-2 border-white dark:border-slate-700 flex-shrink-0" />
            <div className="min-w-0">
              ...
              <div className="text-xs font-bold text-indigo-600 dark:text-indigo-400 truncate mt-0.5">{article.profiles?.professional_role || 'Creator & Contributor'}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 text-xs font-black text-slate-500 dark:text-slate-400 sm:ml-auto flex-wrap">
            <span className="bg-white dark:bg-slate-800 px-3.5 py-2 rounded-xl border border-slate-200/80 dark:border-slate-700 shadow-sm flex items-center gap-1.5">
              <Eye className="w-4 h-4 text-indigo-500" /> {(article.pageviews || 1).toLocaleString()} Hits
            </span>
        ...
        {/* Featured Media */}
        {article.image_url && (
          <img src={article.image_url} alt={article.title} className="w-full h-72 sm:h-[480px] object-cover rounded-3xl mb-14 shadow-2xl border border-slate-200 dark:border-slate-800 flex-shrink-0" />
        )}
        ...
        <DiscussionThread articleSlug={slug} />
        ...
        <div className="mt-24 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white rounded-3xl p-8 sm:p-14 shadow-2xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex-1">
            ...
            <h3 className="text-2xl sm:text-3xl font-black mb-2">Enjoyed this perspective?</h3>
            <p className="text-slate-300 text-sm mb-0">Whether you are writing personal life stories, philosophy, culinary guides, or tech architectures, our real-time AI Reach Assistant helps you polish your stories and drop live instantly.</p>
          </div>
