...
import { Eye, Clock, Trash2, Edit3, Bookmark, Users, Sparkles, LayoutDashboard, Rocket, ArrowRight } from 'lucide-react';
...
export default function AuthorDashboard() {
  ...
  const [activeTab, setActiveTab] = useState('stories'); // stories vs bookmarks vs following
      ...
      if (articles.length === 0) {
        try {
          ...
          if (local) {
            ...
            setArticles(activeUser ? parsed.filter(a => a.author_id === activeUser.id || a.author_id === '00000000-0000-0000-0000-000000000000') : parsed);
          }
        } catch(e) {}
      }
  ...
  if (loading) {
    return <div className="flex-1 py-20 text-center font-black text-lg">⚡ Synchronizing Creator Hub & Reading List...</div>;
  }
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-10 bg-white dark:bg-slate-900 p-7 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-black text-slate-900 dark:text-white">Creator<span className="text-indigo-600 dark:text-indigo-400">Social</span> Hub</span>
            </div>
            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm font-semibold mt-1">
              Live engagement captures, multi-genre written dispatches, and saved reading vaults for <span className="text-indigo-600 dark:text-indigo-400 font-bold">{user?.user_metadata?.full_name || user?.email || 'Community Storyteller'}</span>.
            </p>
          </div>
          
          <Link href="/studio" className="btn btn-primary px-7 py-3 rounded-2xl font-black text-sm shadow-lg shadow-indigo-600/30 flex items-center gap-2 flex-shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12 text-left">
          ...
          <div className="card p-7 border-l-4 border-indigo-600 bg-white dark:bg-slate-900 flex flex-col justify-between shadow-lg">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400 font-bold">Across {articles.length} published dispatches</div>
          </div>
          ...
          <div className="card p-7 border-l-4 border-amber-500 bg-white dark:bg-slate-900 flex flex-col justify-between shadow-lg">
            ...
            <div className="text-4xl font-black text-slate-900 dark:text-white my-3 font-mono">
              {totalClaps.toLocaleString()} <span className="text-sm font-extrabold text-amber-500 font-sans">Claps</span>
            </div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-bold">Audience resonance hits</div>
          </div>
          ...
          <div className="card p-7 border-l-4 border-emerald-500 bg-white dark:bg-slate-900 flex flex-col justify-between shadow-lg">
            ...
            <div className="text-4xl font-black text-emerald-600 dark:text-emerald-400 my-3 font-mono">
              {bookmarks.length} <span className="text-sm font-extrabold text-slate-500 dark:text-slate-400 font-sans">Bookmarked</span>
            </div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-bold">Following {following.length} active peers</div>
          </div>
          ...
          {activeTab === 'stories' && (
            <div>
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
                ...
                <span className="text-xs font-extrabold text-slate-400">Fully Indexable & Public</span>
              </div>
              ...
              {articles.length === 0 ? (
                <div className="py-16 text-center space-y-4">
                  ...
                  <h4 className="text-xl font-black text-slate-900 dark:text-white">Your creator vault is empty</h4>
                  <p className="text-xs text-slate-500 max-w-md mx-auto">You have not drafted or published any stories yet. Unleash your creative authority and write your first piece!</p>
                  ...
                  {articles.map((art) => (
                    ...
                    >
                      <div className="flex items-center gap-5 flex-1 min-w-0">
                        <img src={art.image_url} alt="Banner" className="w-16 h-16 rounded-2xl object-cover border border-slate-200 dark:border-slate-700 hidden md:block flex-shrink-0" />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            ...
                            <span className="text-xs font-bold text-slate-500">{art.published_at || 'Just now'}</span>
                          </div>
                          ...
                          <div className="flex items-center gap-4 text-xs text-slate-500 font-bold flex-wrap">
                            <span>🔑 Topic: <span className="font-extrabold text-indigo-600 dark:text-indigo-400">{art.target_keyword}</span></span>
                            <span>•</span>
                            <span className="text-amber-600 dark:text-amber-400">👏 {art.claps || 0} Claps</span>
                            <span>•</span>
                            <span className="text-indigo-500">⚡ {art.seo_score} Reach Points</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2.5 self-end sm:self-center flex-shrink-0 flex-wrap">
                        <div className="text-right mr-2 hidden sm:block">
                          <div className="text-base font-black text-indigo-600 dark:text-indigo-400 font-mono">{(art.pageviews || 1).toLocaleString()}</div>
          ...
          {activeTab === 'bookmarks' && (
            <div>
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
                ...
                <span className="text-xs font-extrabold text-slate-400">Stored in browser cache</span>
              </div>

              {bookmarks.length === 0 ? (
                <div className="py-16 text-center space-y-4">
                  <div className="w-16 h-16 rounded-3xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-2xl mx-auto">🔖</div>
                  <h4 className="text-xl font-black text-slate-900 dark:text-white">Your Reading List is empty</h4>
                  <p className="text-xs text-slate-500 max-w-md mx-auto">When exploring articles, click the <b>&quot;Save&quot;</b> bookmark icon on the floating action deck to save your favorite deep dives here for later!</p>
                  <Link href="/" className="btn btn-primary px-6 py-3 rounded-2xl text-xs font-black shadow-md">
          ...
          {activeTab === 'following' && (
            <div>
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
                ...
                <span className="text-xs font-extrabold text-slate-400">Open Community Network</span>
              </div>

              {following.length === 0 ? (
                <div className="py-16 text-center space-y-4">
                  <div className="w-16 h-16 rounded-3xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-2xl mx-auto">👥</div>
                  <h4 className="text-xl font-black text-slate-900 dark:text-white">You are not following any creators yet</h4>
                  <p className="text-xs text-slate-500 max-w-md mx-auto">When reading an inspiring personal story or engineering deep dive, click the <b>&quot;➕ Follow&quot;</b> button next to the author&apos;s name to construct your peer network!</p>
                  <Link href="/" className="btn btn-primary px-6 py-3 rounded-2xl text-xs font-black shadow-md">
