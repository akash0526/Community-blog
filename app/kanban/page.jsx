...
import { Plus, Calendar, ArrowRight, ArrowLeft, Tag } from "lucide-react";
...
function buildKanbanCard({ title, statusId, keyword, priority }) {
	const now = new Date();
	...
	return {
		...
		priority,
		date:
			statusId === "scheduled" || statusId === "published"
				? formattedDate
				: undefined,
	};
}
...
export default function WorkflowKanban() {
	...
	const handleCreateCard = (statusId) => {
		...
		const nextCard = buildKanbanCard({
			title: newTitle.trim(),
			statusId,
				...
				<div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-10 bg-white dark:bg-slate-900 p-7 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm text-left">
					<div>
						<div className="flex items-center gap-2">
							<span className="w-8 h-8 rounded-xl bg-indigo-600 text-white flex items-center justify-center text-xs font-black shadow-md shadow-indigo-500/25">
								📅
							</span>
						...
						<p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm font-semibold mt-1">
							Visually plan long-tail keyword clusters, orchestrate your
							publishing pipelines, and synchronize peer topics.
						</p>
					</div>
					...
					<div className="flex items-center gap-2">
						...
						<Link
							href="/studio"
							className="btn btn-primary px-5 py-2 text-xs font-black shadow-md shadow-indigo-600/25"
						>
							⚡ Open Writing Studio
						</Link>
					</div>
					...
					{swimlanes.map((lane) => {
						...
						return (
							...
							>
								{/* Lane Head */}
								<div
									className={`flex items-center justify-between pb-3.5 mb-4 border-b-2 ${lane.color}`}
								>
									<span className={`font-black text-sm ${lane.text}`}>
										{lane.label}
									</span>
										...
										<div className="flex items-center justify-between gap-2">
											...
											<div className="flex gap-1">
												<button
													...
													className="btn btn-primary px-3 py-1 text-xs font-black"
												>
													Save
												</button>
												<button
											...
											<div>
												...
												<div className="flex items-center justify-between gap-2 mb-2.5 flex-wrap">
													...
													<button
														...
														className="opacity-0 group-hover:opacity-100 text-rose-500 hover:text-rose-700 font-bold text-xs transition"
													>
														× Delete
													</button>
												</div>
