    ...
    try {
      if (isSignUp) {
        ...
        if (data.session) {
          ...
          setMessage('⚡ Confirmation email sent! Check your inbox or use Demo Login below.');
        }
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-8 shadow-2xl relative overflow-hidden">
        ...
        {/* Close Button */}
        <button onClick={onClose} className="absolute top-6 right-6 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500 transition">
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="text-left mb-6">
          <div className="flex items-center gap-2 mb-2">
            <span className="w-8 h-8 rounded-xl bg-gradient-to-tr from-indigo-600 to-violet-600 text-white flex items-center justify-center text-sm font-black shadow-md shadow-indigo-500/25">⚡</span>
            <span className="text-xl font-black text-slate-900 dark:text-white">Apex<span className="text-indigo-600 dark:text-indigo-400 font-black">Community</span></span>
          </div>
          <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">
            {isSignUp ? 'Join the Open Ledger' : 'Welcome Back, Architect'}
          </h3>
          <p className="text-slate-500 dark:text-slate-400 text-xs font-semibold mt-1">
            {isSignUp ? 'Create your profile to write and rank extreme SEO dispatches.' : 'Log in to write dispatches and manage your keyword authority pipelines.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Secure Password</label>
            <div className="relative">
          ...
          >
            {loading ? 'Authenticating Cloud...' : (isSignUp ? 'Sign Up to Open Ledger' : 'Log In Live')}
          </button>
        </form>
        ...
        <div className="my-6 flex items-center gap-3">
          ...
          <span className="text-[11px] font-extrabold uppercase tracking-widest text-slate-400">Or For Instant Testing</span>
          <div className="flex-1 h-px bg-slate-100 dark:bg-slate-800"></div>
        </div>
        ...
        <div className="mt-6 text-center text-xs font-bold text-slate-500">
          {isSignUp ? (
            <span>Already have an author ledger? <button onClick={() => setIsSignUp(false)} className="text-indigo-600 dark:text-indigo-400 font-black underline ml-1">Log In</button></span>
          ) : (
            <span>New to open developer dispatches? <button onClick={() => setIsSignUp(true)} className="text-indigo-600 dark:text-indigo-400 font-black underline ml-1">Sign Up</button></span>
          )}
        </div>
