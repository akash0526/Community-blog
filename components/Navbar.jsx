...
import { Sun, Moon, Edit3, User, LogOut, Menu, X, LayoutDashboard } from 'lucide-react';
    ...
    <>
      <header className="sticky top-0 z-40 h-20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition">
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          {/* Brand */}
          <Link href="/" className="flex items-center gap-3 font-black text-2xl text-slate-900 dark:text-white group">
            <span className="w-9 h-9 rounded-2xl bg-gradient-to-tr from-indigo-600 to-violet-600 text-white flex items-center justify-center text-base font-black shadow-lg shadow-indigo-500/25 group-hover:scale-105 transition transform">⚡</span>
            <span>Apex<span className="text-indigo-600 dark:text-indigo-400 font-black">Community</span></span>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-8 font-extrabold text-sm text-slate-600 dark:text-slate-300">
            ...
            <Link 
              href="/kanban" 
              className={`hover:text-indigo-600 dark:hover:text-indigo-400 transition ${pathname === '/kanban' ? 'text-indigo-600 dark:text-indigo-400 font-black' : ''}`}
            >
              📅 Workflow Kanban
            </Link>
          </nav>
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <div className="relative">
                ...
                >
                  <img src={user?.user_metadata?.avatar_url || 'https://api.dicebear.com/7.x/bottts/svg?seed=Arch'} alt="Avatar" className="w-7 h-7 rounded-full object-cover border border-white dark:border-slate-700 flex-shrink-0" />
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl py-3 text-left animate-fadeIn">
                    <div className="px-5 py-2 border-b border-slate-100 dark:border-slate-800 mb-1">
                      <div className="text-[10px] font-black uppercase text-indigo-600 dark:text-indigo-400 tracking-wider">Active Author</div>
                      <div className="text-xs font-bold text-slate-500 truncate">{user.email || 'Author Session'}</div>
                    </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-6 shadow-2xl space-y-4 font-extrabold text-sm text-left animate-fadeIn">
            <Link href="/" onClick={() => setMobileMenuOpen(false)} className="block py-2 text-slate-800 dark:text-slate-200 hover:text-indigo-600">Explore Feed</Link>
            <Link href="/kanban" onClick={() => setMobileMenuOpen(false)} className="block py-2 text-slate-800 dark:text-slate-200 hover:text-indigo-600">📅 Workflow Kanban</Link>
            {user && (
              <Link href="/dashboard" onClick={() => setMobileMenuOpen(false)} className="block py-2 text-indigo-600 dark:text-indigo-400">📊 My Author Dashboard</Link>
            )}
          </div>
        )}
      </header>
